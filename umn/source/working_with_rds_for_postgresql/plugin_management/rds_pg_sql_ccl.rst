...
This extension provides two SQL throttling methods:
...
#. You can obtain the query ID of a SQL statement with bind variables using the pg_stat_statements extension. Then, you can use **add_ccl_rule_by_queryid** to create rules. For details, see "SQL throttling for Statements with Bind Variables".
...
SQL throttling for Statements Without Bind Variables
----------------------------------------------------
...
#. Toggle on the SQL throttling switch |image1|. SQL throttling rules take effect only after SQL throttling is enabled.
...
SQL throttling for Statements with Bind Variables
-------------------------------------------------
...
Such SQL statements can only be limited by executing them first and then adding SQL throttling rules.
...
#. Add a SQL throttling rule based on the query ID.
...
.. table:: **Table 1** Parameter description
   ...
   +===================================+===========+===============+===============+===============+==================================================================================================================================================================================================================+
   | rds_pg_sql_ccl.enable_ccl         | bool      | false         | ``-``         | ``-``         | Whether to enable a SQL throttling rule.                                                                                                                                                                         |
   +-----------------------------------+-----------+---------------+---------------+---------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | rds_pg_sql_ccl.max_enabled_rules  | int       | 5000          | 500000        | 0             | The number of rules that take effect at the same time.                                                                                                                                                           |
   +-----------------------------------+-----------+---------------+---------------+---------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | rds_pg_sql_ccl.max_concurrent_sql | int       | -1            | 50000         | -1            | The number of SQL statements that can be concurrently executed (its priority is higher than that of SQL throttling rules). If the value is less than or equal to 0, the number of SQL statements is not limited. |
   +-----------------------------------+-----------+---------------+---------------+---------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
.. table:: **Table 2** Function interface description
   ...
   +-------------+----------------------------------------+-----------------------------------+--------------+------------------------------------------------------+
   | 2           | rds_pg_sql_ccl.add_ccl_rule_by_query   | query_string text,                | ruleid       | Adds a SQL throttling rule using SQL statements.     |
   |             |                                        |                                   |              |                                                      |
   ...
   |             |                                        | search_path text default 'public' |              |                                                      |
   +-------------+----------------------------------------+-----------------------------------+--------------+------------------------------------------------------+
   | 3           | rds_pg_sql_ccl.add_ccl_rule_by_queryid | query_id bigint,                  | ruleid       | Adds a SQL throttling rule based on the query ID.    |
   |             |                                        |                                   |              |                                                      |
   |             |                                        | max_concurrency int default 0,    |              |                                                      |
   |             |                                        |                                   |              |                                                      |
   |             |                                        | max_waiting int default 0,        |              |                                                      |
   ...
   |             |                                        | search_path text default 'public' |              |                                                      |
   +-------------+----------------------------------------+-----------------------------------+--------------+------------------------------------------------------+
   | 5           | rds_pg_sql_ccl.enable_ccl_rule         | rule_id bigint                    | bool         | Enables a SQL throttling rule based on the rule ID.  |
   +-------------+----------------------------------------+-----------------------------------+--------------+------------------------------------------------------+
   | 6           | rds_pg_sql_ccl.disable_ccl_rule        | rule_id bigint                    | bool         | Disables a SQL throttling rule based on the rule ID. |
   +-------------+----------------------------------------+-----------------------------------+--------------+------------------------------------------------------+
   | 7           | rds_pg_sql_ccl.disable_all_ccl_rule    | ``-``                             | void         | Disables all rules.                                  |
   +-------------+----------------------------------------+-----------------------------------+--------------+------------------------------------------------------+
   | 8           | rds_pg_sql_ccl.delete_ccl_rule         | rule_id bigint                    | void         | Deletes a SQL throttling rule based on the rule ID.  |
   +-------------+----------------------------------------+-----------------------------------+--------------+------------------------------------------------------+
   | 9           | rds_pg_sql_ccl.update_ccl_rule         | new_rule_id bigint,               | void         | Updates a SQL throttling rule based on the rule ID.  |
   |             |                                        |                                   |              |                                                      |
...
.. table:: **Table 3** View interface description
   ...
   +=================+==========================================+==============================+===================================================================================================================================+
   | 1               | rds_pg_sql_ccl.get_all_enabled_rule      | dbid oid,                    | Displays all SQL throttling rules applied.                                                                                        |
   |                 |                                          |                              |                                                                                                                                   |
   |                 |                                          | queryid bigint,              |                                                                                                                                   |
   |                 |                                          |                              |                                                                                                                                   |
   |                 |                                          | max_concurrency int,         |                                                                                                                                   |
   ...
   |                 |                                          | dbid oid                     |                                                                                                                                   |
   +-----------------+------------------------------------------+------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+
   | 3               | rds_pg_sql_ccl.get_current_db_ccl_rule   | rule_id bigint,              | Displays the SQL throttling rules created for the current database (whether applied or not).                                      |
   |                 |                                          |                              |                                                                                                                                   |
   |                 |                                          | query_id bigint,             |                                                                                                                                   |
   |                 |                                          |                              |                                                                                                                                   |
   |                 |                                          | query_string,                |                                                                                                                                   |
...
.. |image1| image:: /_static/images/en-us_image_0000002534464226.png
